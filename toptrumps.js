let africaAnimal = [
    ...
    [animal13 = "Ropi", speed=70, strength= 50, compassion= 30, likeability= 10, aggression=30],
...
let game = prompt(`Hello ${person}! What deck would you like to play with? 1. African Animals or 2. South American Animals? 
(Enter 1 or 2)`)

if (game == 1) {alert (`Hello ${person}! Let's play African Animal Top Trumps!`)
africaAnimal= shuffle(africaAnimal)
playerOneDeck = africaAnimal.slice(0, 15);
playerTwoDeck = africaAnimal.slice(15, 30)}

if (game == 2){alert (`Hello ${person}! Let's play South American Animal Top Trumps!`)
...
while (playerOneDeck.length > 0 && playerTwoDeck.length > 0){
    ...
    if (stat == 1){
        ...
        vs Player Two has drawn ${card2[0]} Speed: ${card2[1]}`)
    } if (stat == 2){
        alert (`We are comparing Strength. ${card1[0]} Strength: ${card1[2]} 
        vs Player Two has drawn ${card2[0]} Strength: ${card2[2]}`)
    } if (stat == 3){
        alert (`We are comparing Compassion. ${card1[0]} Compassion: ${card1[3]} 
        vs Player Two has drawn ${card2[0]} Compassion: ${card2[3]}`)
    } if (stat == 4){
        alert (`We are comparing Likeability. ${card1[0]} Likeability: ${card1[4]} 
        vs Player Two has drawn ${card2[0]} Likeability: ${card2[4]}`)
    } if (stat == 5){
        alert (`We are comparing Agression. ${card1[0]} Aggression: ${card1[5]} 
        vs Player Two has drawn ${card2[0]} Aggression: ${card2[5]}`)
    }

    if (card1[stat] > card2[stat]) {
        playerOneDeck.unshift(card2), playerOneDeck.unshift(card1); alert(`${card1[0]} beats ${card2[0]}. ${person} WINS.`)
        if (limbo.length > 0) {playerOneDeck = limbo.concat(playerOneDeck), limbo = []; alert(`${person} has won all cards in the limbo pile`)
    }
    alert(`${person} now has ${playerOneDeck.length} cards, Player Two now has ${playerTwoDeck.length} cards`)
    } else if (card2[stat] > card1[stat]){
        playerTwoDeck.unshift(card1), playerTwoDeck.unshift(card2); alert(`${card2[0]} beats ${card1[0]}. Player Two WINS.`)
        if (limbo.length > 0) {playerTwoDeck = limbo.concat(playerTwoDeck); limbo = []; alert(`Player Two has won all cards in the limbo pile`)}
    alert (`${person} now has ${playerOneDeck.length} cards, Player Two now has ${playerTwoDeck.length} cards`)
    }
    else if (card1[stat] = card2 [stat]) {limbo.push(card2), limbo.push(card1)
